...
import { Spinner, InputField, Logo, BackButton } from '../components/common';
    ...
    if (newBusiness) {
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
                <div className="w-full max-w-md bg-white rounded-[3rem] p-12 text-center space-y-10 animate-in zoom-in-95 duration-500 border border-slate-100">
                    ...
                    <div className="space-y-4">
                        <h1 className="text-4xl font-black text-[#163a24] tracking-tighter leading-none">Your Royal Portal is Ready</h1>
                    ...
                    <a href="/business/login" className="block w-full bg-[#2bee6c] text-[#163a24] font-black py-5 rounded-[1.5rem] shadow-xl shadow-green-100 hover:opacity-90 transition-all active:scale-95">Go to Login</a>
                </div>
            </div>
        );
    }
            ...
            <div className="relative hidden lg:flex lg:w-1/2 flex-col justify-between bg-[#163a24] p-16 text-white overflow-hidden">
                ...
                <div className="relative z-10 max-w-lg">
                    ...
                    <div className="space-y-6 text-slate-400 text-lg font-medium">
                        <p className="flex items-center gap-4"><span className="size-8 rounded-full bg-[#2bee6c]/20 text-[#2bee6c] flex items-center justify-center font-bold">1</span> Account Creation</p>
                        <p className={`flex items-center gap-4 ${step > 1 ? 'text-white' : ''}`}><span className={`size-8 rounded-full flex items-center justify-center font-bold ${step > 1 ? 'bg-[#2bee6c] text-[#163a24]' : 'bg-[#1e462e] text-[#4c9a66]'}`}>2</span> Brand Configuration</p>
                        <p className={`flex items-center gap-4 ${step > 2 ? 'text-white' : ''}`}><span className={`size-8 rounded-full flex items-center justify-center font-bold ${step > 2 ? 'bg-[#2bee6c] text-[#163a24]' : 'bg-[#1e462e] text-[#4c9a66]'}`}>3</span> Launch Program</p>
                    </div>
                </div>
            ...
            <div className="flex-grow flex flex-col justify-center px-8 lg:px-24 py-16 bg-white">
                <div className="max-w-[440px] w-full mx-auto space-y-12 animate-in slide-in-from-right-10 duration-700">
                    <div className="space-y-3">
                        <p className="text-[#2bee6c] font-black uppercase tracking-[0.3em] text-[10px]">Onboarding Step {step}</p>
                        <h2 className="text-4xl font-black text-[#163a24] tracking-tighter">{step === 1 ? 'Whats your brand called?' : step === 2 ? 'Direct contact email' : 'Secure your gateway'}</h2>
                    </div>
                    ...
                    <form onSubmit={handleNext} className="space-y-10">
                        ...
                        <div className="flex flex-col gap-4">
                            <button 
                                type="submit" 
                                disabled={loading}
                                className="w-full bg-[#163a24] text-[#2bee6c] font-black py-5 rounded-[1.5rem] shadow-xl shadow-green-100 hover:opacity-90 transition-all flex items-center justify-center active:scale-95 disabled:bg-slate-200"
                            >
                                {loading ? <Spinner className="size-6 text-[#2bee6c]" /> : (step === 3 ? t('createAccount') : 'Continue Journey')}
                            </button>
                            {step > 1 && <button type="button" onClick={() => setStep(step - 1)} className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] hover:text-[#163a24] transition-colors py-2">Go Back</button>}
                        </div>
                    </form>

                    <div className="pt-12 border-t border-slate-50 text-center">
                        <p className="text-sm text-slate-500 font-medium">Already managing a program? <a href="/business/login" className="text-[#2bee6c] font-black ml-1">Log in here</a></p>
                    </div>
                </div>
            </div>
        </div>
